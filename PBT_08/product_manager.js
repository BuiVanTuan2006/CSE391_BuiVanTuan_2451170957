...
// 3. Sắp xếp theo giá (tăng/giảm) - Sử dụng toán tử spread [...] để không làm biến đổi mảng gốc
...
function cheapestByCategory(products) {
    return products.reduce((acc, curr) => {
        ...
        // Nếu danh mục chưa có sản phẩm nào HOẶC sản phẩm hiện tại rẻ hơn sản phẩm đã lưu
        if (!acc[cat] || curr.price < acc[cat].price) {
            acc[cat] = curr;
        }
        return acc;
...
// 7. Tính rating trung bình toàn bộ (Chia tổng điểm cho số lượng sản phẩm)
...
// Kết quả sẽ không có iPad Air vì stock = 0
console.log(getInStock(products));

console.log("\n=== PHONES 15-25 TRIỆU ===");
// Kết quả trả về Samsung S24 và Pixel 9
console.log(filterProducts(products, "phone", 15000000, 25000000));

console.log("\n=== CHEAPEST BY CATEGORY ===");
// Trả về object gom nhóm sản phẩm rẻ nhất của từng danh mục
console.log(cheapestByCategory(products));

console.log("\n=== TOTAL INVENTORY VALUE ===");
// Kết quả: 2.152.050.000đ
console.log(totalInventoryValue(products).toLocaleString("vi-VN") + "đ");

console.log("\n=== FORMATTED PRODUCT LIST ===");
console.log(formatProductList(products.slice(0, 2))); // Demo thử 2 dòng đầu

console.log("\n=== AVERAGE RATING ===");
console.log(averageRating(products)); // Kết quả: 4.5
